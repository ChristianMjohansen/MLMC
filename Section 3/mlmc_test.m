%
% function mlmc_test(mlmc_l, M,L, M0,Eps,Lmin,Lmax, fp, varargin)
%
% multilevel Monte Carlo test routine
%
% [sums cost] = mlmc_l(l,M, varargin)     low-level routine
%
% inputs:  l = level
%          M = number of paths
%
...
function mlmc_test(mlmc_l, M, L, M0, option, Eps, Lmin, Lmax, fp )

%
% first, convergence tests
%

M = 100*ceil(M/100);   % make M a multiple of 100
...
%  disp(sprintf('l = %d',l))
  ...
  parfor j=1:100
    ...
    [sums_j, cst_j] = mlmc_l(l,M/100,option);
    sums = sums + sums_j/M;
    cst  = cst  + cst_j/M;
  end
...
for i = 1:length(Eps)
  ...
  [P, Ml, Cl] = mlmc(mlmc_l,M0,option,eps,Lmin,Lmax,alpha,beta,gamma);          
