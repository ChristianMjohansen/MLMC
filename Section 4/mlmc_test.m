%
% function mlmc_test(mlmc_l, M,L, M0,Eps,Lmin,Lmax, fp)
%
...
function mlmc_test(mlmc_l, M,L , M0, Nq, TOL_Newton, model, option, Eps, Lmin, Lmax, fp )
...
%  disp(sprintf('l = %d',l))
  ...
  parfor j=1:100
    ...
    [sums_j, cst_j] = mlmc_l(l,M/100,NQ,TOL_Newton,model,option);
    sums = sums + sums_j/M;
    cst  = cst  + cst_j/M;
  end
...
PRINTF2(fp,'***************************** \n\n');
PRINTF2(fp,'   eps       value    mlmc_NS_cost   std_mlmc_cost    N_l \n');
PRINTF2(fp,'----------------------------------------------------------- \n');
...
for i = 1:length(Eps)
  eps = Eps(i); 
 %Nq = suboptimal_Nq(eps,L,model,option,TOL_Newton);
  if option == 3 || option == 6 || option == 7
     [P, Ml, Cl] = mlmc_smooth(mlmc_l,M0,1,TOL_Newton,model,option,eps,Lmin,Lmax,alpha,beta,gamma);
     mlmc_cost_smooth = sum(Ml.*Cl);

     PRINTF2(fp,'%.3e %10.3e  %.3e ', ...
    	  eps, P, mlmc_cost_smooth);
     PRINTF2(fp,'%10d',Ml);
     PRINTF2(fp,'\n');
  else
     [P, Ml, Cl] = mlmc_smooth(mlmc_l,M0,Nq(i),TOL_Newton,model,option,eps,Lmin,Lmax,alpha,beta,gamma);      
     %[~, Ml2, Cl2] = mlmc(@level_estimator,M0,model,option,eps,Lmin,Lmax,0,0,0);
     mlmc_cost_smooth = sum(Ml.*Cl);
     %std_mlmc_cost = sum(Ml2.*Cl2);

     PRINTF2(fp,'%.3e %10.3e  %.3e  %.3e ', ...
         eps, P, mlmc_cost_smooth, 1 ); 
     PRINTF2(fp,'%10d',Ml);
     PRINTF2(fp,'\n');
