% function [P, Ml, cost] = mlmc(mlmc_l,M0,eps,Lmin,Lmax,
%                               alpha,beta,gamma)
%
...
%
% [sums, cost] = mlmc_fn(l,M)     low-level routine
%
% inputs:  l = level
%          M = number of samples
%
...
function [P, Ml, Cl] = mlmc(mlmc_l, M0,model,option,eps,Lmin,Lmax, ...
                            alpha0,beta0,gamma0)
...
%
    for l=0:L
      if dMl(l+1) > 0
        [sums, cost] = mlmc_l(l,dMl(l+1),model,option);
...
%
    Ms  = ceil( 2*sqrt(Vl./Cl)*sum(sqrt(Vl.*Cl)) /  eps^2 );
    dMl = max(0, Ms-Ml);
%
